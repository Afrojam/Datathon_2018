...
def prepare_data(data, t_interval=8, num_stations=7):
    for day in days:
        ...
        data_x = data[(data['date'] < str(day)) & (data['date'] >= str(day - t_interval*24))]
        data_y = data[(data['date'] >= str(day)) & (data['date'] < str(day + 1*24))]
        if data_x.shape[0] != (24 * t_interval * num_stations) or data_y.shape[0] != (24*num_stations):
